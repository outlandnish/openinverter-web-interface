...
public:
  ...
  bool isDownloadingJson() const {
    return state_ == JSON_INIT_SENDING || state_ == JSON_INIT_WAITING || state_ == JSON_SEGMENT_SENDING ||
           state_ == JSON_SEGMENT_WAITING;
  }
  bool isAcquiringSerial() const { return state_ == SERIAL_SENDING || state_ == SERIAL_WAITING; }
  ...
  String getSerial() const;     // Get formatted serial string (e.g., "ABCD:1234:5678:9ABC")
  ...
  bool resetToScanningMode();  // Reset CAN filter to scanning mode using stored config
  ...
  // Callbacks
  void setConnectionReadyCallback(ConnectionReadyCallback callback) { connectionReadyCallback_ = callback; }
  void setJsonProgressCallback(JsonDownloadProgressCallback callback) { jsonProgressCallback_ = callback; }
  void setJsonStreamCallback(JsonStreamCallback callback) { jsonStreamCallback_ = callback; }

  ConnectionReadyCallback getConnectionReadyCallback() const { return connectionReadyCallback_; }
  JsonDownloadProgressCallback getJsonProgressCallback() const { return jsonProgressCallback_; }
  JsonStreamCallback getJsonStreamCallback() const { return jsonStreamCallback_; }

  // Rate limiting for parameter requests
  void setParameterRequestRateLimit(unsigned long intervalUs) { minParamRequestIntervalUs_ = intervalUs; }
