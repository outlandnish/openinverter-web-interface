...
class InverterAPI {
  ...
  /**
   * Get parameter JSON definitions from device (for localStorage caching)
   * @param nodeId - Node ID to fetch parameters from a specific device (required for multi-client support)
   * @param signal - AbortSignal for canceling the request
   * @param onProgress - Progress callback for download tracking
   */
  async getParamsJson(
    nodeId: number,
    signal?: AbortSignal,
    ...
    const url = `/params/json?nodeId=${nodeId}`
    const response = await fetch(url, { signal })
    
    // If no progress callback or no streaming support, use simple json parsing
    ...
    if (!total) {
      // If no content-length header, just return the JSON without progress
      return response.json()
    }
    ...
    try {
      ...
      console.error('Error streaming response, falling back to simple fetch:', error)
      // Fallback: refetch without streaming
